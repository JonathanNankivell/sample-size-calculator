...
add_to_csv <- function(numb_of_sims, numb_of_patients, runner, chooser, chooser_params, alpha) {
  ...
  for (i in 1:orders_of_magitude) {
    ...
    write.table(df, "power and ENS for adaptive trials.csv", append=TRUE, sep=",", col.names = FALSE, row.names = FALSE)
  }
}
  ...
  for (y in 1:dim(priors)[1]) {
    if (y >= x) {
      ...
      for (z in 1:dim(choosers)[1]) {
        ...
        add_to_csv(100, 2**9, one_run, choosers[z,][1], as.numeric(choosers[z,][2]), alpha)
      }
    }
  }
}
...
df <- df[df$prior.i == 100,]
df <- df[df$prior.j == 1,]
df <- df[df$prior.k == 3,]
df <- df[df$prior.l == 1,]

# group by allocation method and average across different sims of the same thing
df <- aggregate(df[,10:11], list(paste(df$chooser, df$chooser_params)), mean)

# plot results on a scatter plot
plot(df$power,df$ENS, xlim=c(0,1),ylim=c(0,1), main="Pareto frontier", sub="Expected Power vs Expected Benifit", xlab="Expected Power", ylab="Expected Proportion of Successes")
