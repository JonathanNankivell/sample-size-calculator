...
add_to_csv <- function(numb_of_sims, numb_of_patients, runner, chooser, chooser_params, alpha) {
  ...
  for (i in 1:orders_of_magitude) {
    ...
    write.table(df, "output.csv", append=TRUE, sep=",", col.names = FALSE, row.names = FALSE)
  }
}
...
choosers = rbind(c("balanced","NA"),c("thomper","NA"),c("DTL",0),c("DTL",2),c("DTL",6),c("DBCD",0),c("DBCD",2),c("DBCD",4),c("tw_foo",0.5))

for (i in 1:dim(choosers)[1]) {
  print(paste(choosers[i,][1], choosers[i,][2]))
  add_to_csv(100, 2**9, one_run, choosers[i,][1], as.numeric(choosers[i,][2]), alpha)
}
